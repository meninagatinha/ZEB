...
class ModelRunner:
    ...
    RF_RANDOMIZED_SEARCH_GRID_PARAMS = dict(n_estimators=N_ESTIMATORS, max_features=MAX_FEATURES,
                                            max_depth=RF_MAX_DEPTH, min_samples_split=MIN_SAMPLES_SPLIT,
                                            min_samples_leaf=MIN_SAMPLES_LEAF, bootstrap=[True, False])
    SVR_GRID_SEARCH_PARAMS = dict(kernel=['rbf', 'sigmoid'], C=np.logspace(np.log10(0.001), np.log10(200), num=10),
                                  gamma=np.logspace(np.log10(0.00001), np.log10(2), num=20))
    XGBOOST_GRID_SEARCH_PARAMS = dict(objective=['reg:squarederror'], learning_rate=[0.1], n_estimators=N_ESTIMATORS,
                                      max_depth=RF_MAX_DEPTH, min_child_weight=[4], colsample_bytree=[0.7])
    ...
    def run_svr(self):
        ...
        """
        svr = SVR()
        grid_searcher = GridSearchCV(svr,
                                     self.SVR_GRID_SEARCH_PARAMS,
                                     n_jobs=self.N_JOBS,
                                     verbose=self.VERBOSE,
                                     cv=self.CV)
    ...
    def run_xgboost(self):
        ...
        xgb_grid_search = GridSearchCV(xgb,
                                       self.XGBOOST_GRID_SEARCH_PARAMS,
                                       cv=self.CV,
                                       n_jobs=self.N_JOBS,
                                       verbose=True)
